...
| 3 | `subscriber_read` | Read a single subscriber or list with optional filters |
| 4 | `subscriber_create` | Create new subscriber (merged with defaults) |
| 5 | `subscriber_delete` | Delete a subscriber by IMSI |
| 6 | `subscriber_update_profile` | Update profile params (security, AMBR, status, restrictions, etc.) |
| 7 | `subscriber_update_slices` | Update slice/session (DNN) configuration |
| 8 | `list_ue_sessions` | Live UE registrations and PDU sessions from AMF + SMF |
| 9 | `tail_nf_logs` | Filtered log reads across one or more NFs, interleaved by timestamp |
| 10 | `read_nf_config` | Read YAML config from any NF |
| 11 | `get_ue_trace` | E2E UE call flow reconstruction across all NFs |
| 12 | `amf_ran_query` | RAN state: connected gNBs, registered UEs, slices |
| 13 | `nf_resource_usage` | CPU/RAM/I/O utilisation per NF |
...
```
subscriber_create(imsi="999700000000002",
...
│       ├── subscriber_read.py
│       ├── subscriber_create.py
│       ├── subscriber_delete.py
