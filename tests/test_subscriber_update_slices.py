...
class TestValidation:
    def test_invalid_imsi(self):
        r = subscriber_update_slices(imsi="bad", slices=_VALID_SLICE)
        assert r["ok"] is False

    def test_not_a_list(self):
        r = subscriber_update_slices(imsi=IMSI, slices={"sst": 1})
        assert r["ok"] is False
        assert "list" in r["error"]

    def test_empty_slices(self):
        r = subscriber_update_slices(imsi=IMSI, slices=[])
        assert r["ok"] is False
        assert "empty" in r["error"]

    def test_slice_not_a_dict(self):
        r = subscriber_update_slices(imsi=IMSI, slices=["not-a-dict"])
        assert r["ok"] is False

    def test_slice_missing_sst(self):
        r = subscriber_update_slices(
            imsi=IMSI,
    ...
    def test_empty_session_array(self):
        r = subscriber_update_slices(imsi=IMSI, slices=[{"sst": 1, "session": []}])
        assert r["ok"] is False
        assert "empty" in r["error"]

    def test_session_not_a_dict(self):
        r = subscriber_update_slices(imsi=IMSI, slices=[{"sst": 1, "session": ["bad"]}])
        assert r["ok"] is False

    def test_session_missing_name(self):
        r = subscriber_update_slices(imsi=IMSI, slices=[{"sst": 1, "session": [{"type": 3}]}])
        assert r["ok"] is False
    ...
    def test_subscriber_not_found(self):
        with patch("tools.subscriber_update_slices.get_subscribers_col") as mc:
            ...
            r = subscriber_update_slices(imsi=IMSI, slices=_VALID_SLICE)
        assert r["ok"] is False
...
class TestHappyPath:
    ...
    def test_two_dnns(self, mock_get_col):
        col = make_mock_col([make_subscriber(IMSI)])
        mock_get_col.return_value = col
        r = subscriber_update_slices(imsi=IMSI, slices=_TWO_DNNs)
        assert r["ok"] is True
    ...
    def test_supi_format_accepted(self, mock_get_col):
        col = make_mock_col([make_subscriber(IMSI)])
        mock_get_col.return_value = col
        r = subscriber_update_slices(imsi=f"imsi-{IMSI}", slices=_VALID_SLICE)
        assert r["ok"] is True
