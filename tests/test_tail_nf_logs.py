...
from conftest import log_line, write_nf_log, unwrap
...
class TestHappyPath:
    def test_single_nf_returns_lines(self, log_dir):
        ...
        r = unwrap(tail_nf_logs(nf="amf", level="info", lines=10))
        assert r["ok"] is True
    ...
    def test_all_nf_merges_results(self, log_dir):
        ...
        r = unwrap(tail_nf_logs(nf="all", level="info"))
        assert r["ok"] is True
    ...
    def test_list_of_nfs(self, log_dir):
        ...
        r = unwrap(tail_nf_logs(nf=["amf", "ausf"]))
        assert r["ok"] is True
    ...
    def test_results_sorted_chronologically(self, log_dir):
        ...
        r = unwrap(tail_nf_logs(nf="amf", level="info"))
        assert r["ok"] is True
    ...
    def test_lines_cap_respected(self, log_dir):
        ...
        r = unwrap(tail_nf_logs(nf="amf", lines=5))
        assert r["ok"] is True
...
class TestFiltering:
    ...
    def test_level_error_only(self, log_dir):
        ...
        r = unwrap(tail_nf_logs(nf="amf", level="error"))
        assert r["ok"] is True
    ...
    def test_grep_keyword(self, log_dir):
        ...
        r = unwrap(tail_nf_logs(nf="amf", grep="Registration"))
        assert r["ok"] is True
    ...
    def test_grep_regex(self, log_dir):
        ...
        r = unwrap(tail_nf_logs(nf="amf", grep="PDU|Registration"))
    ...
    def test_since_relative_filters_old_lines(self, log_dir):
        ...
        r = unwrap(tail_nf_logs(nf="amf", since="1m"))
        assert r["ok"] is True
        msgs = [e["message"] for e in r["lines"]]
...
class TestErrorHandling:
    ...
    def test_one_nf_error_doesnt_block_others(self, log_dir):
        ...
        r = unwrap(tail_nf_logs(nf=["amf", "smf"]))
        assert r["ok"] is True
